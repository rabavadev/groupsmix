...
-- Drop the UNIQUE constraint (which automatically drops its backing index).
ALTER TABLE sites DROP CONSTRAINT IF EXISTS sites_user_id_key;

-- Drop the index too in case it was created independently.
DROP INDEX IF EXISTS sites_user_id_key;

-- board_order column for ordering boards per user. Default 0 for existing rows.
